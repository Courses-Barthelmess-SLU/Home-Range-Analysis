...
## Set up - Prepping the R workspace and the data
...
pacman::p_load(tidyverse, adehabitatHR, osmdata,adehabitatLT, sf, sp, magrittr,ggmap,here, leaflet) #checks to see if packages are installed and installs them if not
...
```{r}
CapeBuffalo <- read.csv(here("Data", "CapeBuffalo", "Kruger_African_Buffalo_GPS_tracking.csv"))
```
...
```{r}
plot(CapeBuffalo$'utm_easting', CapeBuffalo$'utm_northing', asp = 1)
```

```{r}
#subset DF to get just animal name, X and Y coords

Data <- CapeBuffalo %>% dplyr::select(utm_easting, utm_northing, individual_local_identifier)
...
```{r}
sp::coordinates(Data)<-Data[,c("utm_easting", "utm_northing")]
data1 <-st_as_sf(Data, coords = c("utm_easting", "utm_northing"))
```

Ok! Our data are ready, so we can move on to Part 2!

## Part 1 - Get a base map

We'll get a base map of Kruger National Park and environs on which to plot our home ranges, since these data come from that area. We use a special R package for using data from \[Open Street Map\](<https://www.openstreetmap.org/#map=8/-23.941/30.498>) to do so.

Create a bounding box for Kruger National Park, add primary highways, and save as an sf object.

```{r}
Kruger_bb <-getbb("Kruger National Park")
Kruger_highway <-getbb("Kruger National Park") |> opq() |> add_osm_feature(key = "highway", value = c("primary")) |> osmdata_sf()
```
...
```{r}
ggplot() +
  geom_sf(data = Kruger_highway$osm_lines)
```

```{r}
kruger_map <- leaflet() |> addTiles() |> setView(lng = 31.47, lat = -23.57, zoom=8)
kruger_map
```

## Part 2 - Calculate the MCP home ranges for the buffalo.

First, let's see how many buffalo are in our dataset by examining the 'individual_local_identifer (aka Name). To do so, we need to change the names from character variables to factor variables.

```{r}
#change from character to factor
Data$individual_local_identifier<-as.factor(Data$individual_local_identifier)

#now see how many levels of the factor there are
levels(Data$individual_local_identifier)
```
...
mcp100 <-st_as_sf(mcp_est100, crs = '+proj=longlat +datum=WGS84', fillColor = "purple")
...
```{r}
kruger_map |> addPolygons(data = mcp100, fillColor = "purple")
```

Now use the sf package to write the shapefiles.

```{r}
sf::st_write(as(mcp_est100, "sf"), here("Data", "Shapefiles", "MCP100.shp"))

sf::st_write(as(mcp_est90, "sf"), here("Data", "Shapefiles", "MCP90.shp"))

sf::st_write(as(mcp_est80, "sf"), here("Data", "Shapefiles", "MCP80.shp"))

sf::st_write(as(mcp_est70, "sf"), here("Data", "Shapefiles", "MCP70.shp"))

#now read them in again as sf objects

mcp100 <- read_sf(here("Data", "Shapefiles", "MCP100.shp"))
mcp100 <-st_transform(mcp100, crs = '+proj=longlat +datum=WGS84')
...
sf::st_write(as(homeranges, "sf"), here("Data", "Shapefiles","KDEhomeranges.shp"))
HRs <-st_as_sf(homeranges, crs = '+proj=longlat +datum=WGS84')
```
And map
```{r}
kruger_map |> addPolygons(data = HRs, fillColor = "red", fill = TRUE)
```
